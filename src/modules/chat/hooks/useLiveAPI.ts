            ...
            if (message.serverContent?.inputTranscription?.finished && inputText) {
              ...
              if (remainingExamsRef.current && remainingExamsRef.current.length > 0 && isAffirmative(inputText)) {
                ...
                if (rest.length > 0) {
                  ...
                  examContextRef.current = ''; // Clear context if all done
                }
                ...
                // Clear remaining if user says something else
                remainingExamsRef.current = [];
              }
                ...
                if (answerKey !== lastExamAnswerKeyRef.current) {
                  try {
                    ...
                    if (examFlowStepRef.current === 0) {
                      // Transition to Step 1 and ask the first question mandatorily
                      ...
                      if (currentSubjects.length === 0 && !wantsAll) {
                        ...
                        if (currentSubjects.length > 0) {
                          ...
                          if (foundRows.length > 0) {
                            ...
                            shouldSend = true;
                            examFlowStepRef.current = 0; // Reset state
                          } else {
                ...
                try {
                  switch (call.name) {
                    ...
                    case 'search_main_exams': {
                      ...
                      // Normalize status, level, semester, department
                      ...
                      if (examFlowStepRef.current === 0) {
                        examFlowStepRef.current = 1;
                        const msg = "Kripya thodi aur jankari dein, main aapki madad karungi: Aap Collegiate student hain ya Non-Collegiate? Aap graduate (UG) hain ya postgraduate (PG)? Aur semester kaunsa hai?";
                        result = {
                          error: "Missing parameters",
                          instruction_to_model: "Ask the user exactly this question:",
                          question_to_ask: msg,
                          verified_answer: msg
                        };
                        break;
                      ...
                      if (examFlowStepRef.current === 1) {
                        if (hasAllDetails) {
                          examFlowStepRef.current = 2;
                          const msg = "Aap kaun se papers ke baare mein jaanna chahte hain? Kya aap mujhe bataengi?";
                          result = {
                            instruction_to_model: "Ask the user exactly this question:",
                            question_to_ask: msg,
                            verified_answer: msg
                      ...
                      if (subject) {
                        ...
                        if (inferred.length > 0) {
                          toolSubjects.push(...inferred);
                        } else {
                          toolSubjects.push(subject);
                        }
                      }
